...
@click.option('--name', required=True, help="Name of the fsm that we want to run.")
...
def main(file, name, inputs, delimiter, interactive):
    ...
    try:
        ...
        if not name:
            click.secho("Error: You must enter the name of the fsm model.", fg="red", bold=True)
            return
        ...
        if interactive:
            ...
            while True:
                valid_inputs = fsm.get_valid_inputs()
                click.secho(f"\nValid inputs for current state '"
                            f"{fsm.current_state}': {valid_inputs}", fg="cyan", bold=True)
                user_input = input(click.style("Enter input symbol: ", fg="yellow", bold=True))
                if user_input.lower() == "exit":
                    break
                elif user_input not in valid_inputs:
                    click.secho(f"Invalid input '{user_input}'. Please enter one of {valid_inputs}.",
                                fg="red", bold=True)
                else:
